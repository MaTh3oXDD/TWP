...
namespace Logic
{
    internal class PoolTable : LogicAbstractAPI
    {
        ...
        {
            // Iterujemy po wszystkich parach piłek
            for (int i = 0; i < _balls.Count; i++)
            {
                // Zaczynamy od i + 1, aby uniknąć sprawdzania piłki z samą sobą
                // i powtarzania tych samych par (piłka A vs B jest tym samym co B vs A)
                for (int j = i + 1; j < _balls.Count; j++)
                {
                    ...
                    float sumOfRadii = ball1.Radius + ball2.Radius-15; // Suma promieni

                    // Sprawdź, czy doszło do kolizji (odległość mniejsza lub równa sumie promieni)
                    // Zmniejszony o -15 bufor z kodu użytkownika został usunięty tutaj,
                    // ponieważ zwykle kolizja występuje, gdy odległość <= sumie promieni.
                    // Jeśli -15 miało specjalne znaczenie, można je dodać z powrotem,
                    // ale może to prowadzić do błędów w detekcji kolizji.

                    // Dodatkowo można dodać warunek sprawdzający, czy piłki poruszają się w kierunku siebie,
                    // aby uniknąć wielokrotnego rozwiązywania tej samej kolizji,
                    // ale podstawowa detekcja odległości też działa.
